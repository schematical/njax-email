...
module.exports = function(options){
	...
	njax_app._registerMiddlware = function(middleware){
		if(!_.isFunction(middleware)){
			throw new Error("Invalid middleware param passed in");
		}
		app.njax.email.middleware.push(middleware);
	}
    njax_app._registerEmail = function(event, options){
        ...
        app.njax.callbacks.listen(event, function(event, data, next){
			...
			for(var i in app.njax.email.middleware){
				data = app.njax.email.middleware(data);
			}
			var accounts = null;
			if(data._accounts){
				...
				async.eachSeries(
					...
					function(account, _cb){
						if(_.isString(account)){
							...
							data._unsub_key = encodeURIComponent(app.njax.crypto.encrypt(data._account._id, app.njax.config.client_secret));
						}
						...
						async.series([
							function(cb){
								...
								return app.njax.email.canSendToUser(account, event, data, function(err, send_to_user){
									if(err) return next(err);
									if(!send_to_user){
	...
	app.njax.email.registerMiddlware = _.bind(njax_app._registerMiddlware, app);
